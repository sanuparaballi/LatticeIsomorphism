...
def elementary_column_op(matrix: np.ndarray) -> np.ndarray:
    """Applies a single random elementary column operation."""
    n = matrix.shape[0]
    i, j = random.sample(range(n), 2)
    r = random.choice([-1, 1])

    op_matrix = np.identity(n, dtype=np.int64)
    op_matrix[i, j] = r
    return matrix @ op_matrix


def generate_positive_basis(dim: int, scramble_steps: int) -> np.ndarray:
    """Generates a valid, non-trivial basis for Z^n."""
    print(f"Generating {dim}D positive test basis...")
    matrix = np.identity(dim, dtype=np.int64)
    for _ in range(scramble_steps):
        matrix = elementary_column_op(matrix)
    print("Positive case generated.")
    return matrix


def generate_negative_basis(dim: int) -> np.ndarray:
    """Generates a basis for a lattice that is not isometric to Z^n."""
    print(f"Generating {dim}D negative test basis...")
    matrix = np.identity(dim, dtype=np.int64)
    matrix[0, 0] = 2  # Makes the determinant 2 (not unimodular)
    print("Negative case generated.")
    return matrix


def main():
    """Main function to generate and save test case basis matrices."""
    DIMENSION = 256
    positive_basis = generate_positive_basis(
        dim=DIMENSION, scramble_steps=DIMENSION * 5)
    negative_basis = generate_negative_basis(dim=DIMENSION)

    output_path = os.path.join(os.path.dirname(
        __file__), f"test_data_basis_{DIMENSION}d.npz")
    ...
    print(f"\nTest case basis matrices successfully saved to: {output_path}")
